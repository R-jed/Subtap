...
- 上游 `v0.2.1` 是 GitHub verified signed tag，对应 commit `31646a4`：[GitHub Release](https://github.com/google/sentencepiece/releases/tag/v0.2.1)。
...
因此现有证据能证明“官方项目有 Apache-2.0 许可证和 signed source tag”，但不能证明“这个精确二进制 wheel 由该 tag 构建”。按当前设计的精确 wheel→source 门禁，该路线必须停止；代理不能用项目自写 JSON 冒充上游 provenance 或人工审批。
