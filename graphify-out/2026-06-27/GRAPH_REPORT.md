...
## Corpus Check
- 96 files · ~37,400 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 1661 nodes · 2754 edges · 97 communities (81 shown, 16 thin omitted)
- Extraction: 74% EXTRACTED · 24% INFERRED · 0% AMBIGUOUS · INFERRED: 659 edges (avg confidence: 0.62)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `fb90a8e4`
...
### Community 3 - "报告生成"
Cohesion: 0.07
Nodes (47): Pipeline, Pipeline orchestrator with stage-based execution., Execute Subtap stages with workspace-backed state.      Each stage reads from an, Run a single pipeline stage., generate_report(), Report generator for subtitle quality analysis., Generate a Markdown report for subtitle quality analysis.      Args:         qua, load_config() (+39 more)
...
### Community 12 - "模型测试"
Cohesion: 0.12
Nodes (22): ModelConfig, Model management configuration., _config_with_model_root(), SubtapConfig, Tests for model management system., CLI models status runs without crash., Config with model root pointing to tmp., CLI models verify runs without crash. (+14 more)
...
### Community 22 - "事件系统"
Cohesion: 0.09
Nodes (30): BaseModel, AudioConfig, OutputConfig, Pydantic v2 config schema with default merge., VAD / silence splitting parameters., Audio extraction and processing settings., Workspace directory settings., Output system configuration. (+22 more)

### Community 23 - "ASR测试"
Cohesion: 0.25
Nodes (5): _get_model_root(), Resolve model root path from config., Get the directory path for a specific model., Path, SubtapConfig
...
### Community 35 - "流水线核心"
Cohesion: 0.06
Nodes (29): Tests for output system., Test writing artifacts., Test finalizing output., Test NamingStrategy generates correct final name., Test old version cleanup., Test OutputEngine initialization., Test writing final output., Test writing metrics. (+21 more)
...
Cohesion: 0.12
Nodes (14): ModelRegistry, Query model status across all registered models., List all available model names., Check if a model is installed and complete., Status shows all models missing when no files exist., Status shows asr installed when files exist., get_path returns correct directory., get_path raises ValueError for unknown model. (+6 more)
...
Cohesion: 0.29
Nodes (4): ModelStatus, Model management system: registry, download, verify., Status of a single model., Check status of all registered models.
...
- **Why does `SubtapConfig` connect `断句测试` to `导出模块`, `分段处理`, `报告生成`, `HTTP ASR后端`, `模型测试`, `媒体处理`, `管道控制`, `配置管理`, `状态管理`, `事件系统`, `ASR测试`, `LLM后端`, `替换规则`, `工作空间`, `日志系统`, `测试配置`, `工程卫生`, `对齐基础`, `进度条`, `Community 84`, `Community 90`, `Community 92`?**
